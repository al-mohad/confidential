...
class AwsSecretsManagerProvider implements RemoteSecretProvider {
  ...
  Future<RemoteSecretValue?> getSecretValue(String name, {String? version}) async {
    ...
    try {
      ...
      final secretValue = RemoteSecretValue(
        value: secretString ?? '',
        binaryValue: secretBinary != null 
            ? base64Decode(secretBinary)
            : null,
        metadata: metadata,
      );
  ...
  Future<Map<String, dynamic>> _makeAwsRequest(String action, Map<String, dynamic> payload) async {
    final region = config.region ?? 'us-east-1';
    final endpoint = config.endpoint ?? 'https://$_service.$region.amazonaws.com';
    ...
    if (response.statusCode == 200) {
      ...
      if (errorCode?.contains('ResourceNotFoundException') == true) {
        throw RemoteSecretNotFoundException(message, errorCode: errorCode, statusCode: response.statusCode);
      } else if (errorCode?.contains('UnauthorizedOperation') == true || response.statusCode == 403) {
        throw RemoteSecretAuthException(message, errorCode: errorCode, statusCode: response.statusCode);
      } else if (response.statusCode == 429) {
        throw RemoteSecretRateLimitException(message, errorCode: errorCode, statusCode: response.statusCode);
      } else {
        throw RemoteSecretException(message, errorCode: errorCode, statusCode: response.statusCode);
      }
    }
  }

  /// Adds AWS Signature Version 4 authentication headers.
  Future<void> _addAwsAuthHeaders(HttpClientRequest request, String payload) async {
