...
class HardwareKeyStorageExample {
  ...
  Future<void> _demonstrateCustomConfig() async {
    ...
    try {
      ...
      final key = await customKeyStorage.generateAndStoreKey(
      ...
      await Future.delayed(Duration(milliseconds: 100)); // Simulate time passage
  ...
  Future<void> _demonstratePlatformFeatures() async {
    ...
    try {
      ...
      print('  - Hardware backing available: ${storageInfo['hardwareBackingAvailable']}');
      print('  - Hardware backing enabled: ${storageInfo['hardwareBackingEnabled']}');
  ...
  Future<void> demonstrateEncryptionIntegration() async {
    ...
    try {
      ...
      if (retrievedKey != null) {
        final decryptedData = _simulateDecryption(encryptedData, retrievedKey.keyData);
