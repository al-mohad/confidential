...
class EnhancedHardwareKeyStorageExample {
  ...
  Future<void> _demonstrateNativeMaxSecurity() async {
    ...
    try {
      ...
      if (storageInfo['platformCapabilities'] != null) {
        final capabilities = storageInfo['platformCapabilities'] as Map<String, dynamic>;
        print('  - StrongBox available: ${capabilities['strongBoxAvailable']}');
        print('  - Secure Enclave available: ${capabilities['secureEnclaveAvailable']}');
  ...
  Future<void> _demonstratePlatformSpecificFeatures() async {
    ...
    try {
      ...
      final securityFeatures = storageInfo['securityFeatures'] as Map<String, dynamic>;
      ...
      print('📋 Platform keys: ${keyIds.where((id) => id.startsWith('platform_')).join(', ')}');
  ...
  Future<void> _demonstrateBiometricAuthentication() async {
    ...
    try {
      ...
      print('  - Biometric available: ${capabilities?['biometricAvailable'] ?? false}');
      print('  - Biometric auth enabled: ${storageInfo['biometricAuthEnabled']}');
  ...
  Future<void> _demonstrateKeyAttestation() async {
    ...
    try {
      ...
      final key = await attestationKeyStorage.generateAndStoreKey(
        keyId: 'attestation_test_key',
        version: 1,
      );
      
      final storageInfo = await attestationKeyStorage.getStorageInfo();
      final attestationInfo = storageInfo['attestation'] as Map<String, dynamic>;
      ...
      if (attestationInfo['supported'] == true) {
        ...
        if (capabilities != null) {
          ...
          print('  - Hardware backing: ${capabilities['hardwareBackingAvailable']}');
          print('  - StrongBox: ${capabilities['strongBoxAvailable']}');
          print('  - Secure Enclave: ${capabilities['secureEnclaveAvailable']}');
        }
  ...
  Future<void> _demonstratePerformanceComparison() async {
    ...
    try {
      ...
      final speedup = fallbackDuration.inMilliseconds / nativeDuration.inMilliseconds;
      if (speedup > 1) {
      ...
      await _cleanupPerformanceKeys(nativeStorage, fallbackStorage, testIterations);
  ...
  Future<void> demonstrateEnhancedEncryptionIntegration() async {
    ...
    try {
      ...
      print('  - Security level: ${capabilities?['bestSecurityLevel'] ?? 'unknown'}');
      print('  - Hardware backing: ${storageInfo['hardwareBackingAvailable']}');
      print('  - Native channels: ${storageInfo['useNativePlatformChannels']}');
      ...
      final encryptedData = _simulateEnhancedEncryption(sensitiveData, keyMaterial);
      ...
      if (retrievedKey != null) {
        final decryptedData = _simulateEnhancedDecryption(encryptedData, retrievedKey.keyData);
